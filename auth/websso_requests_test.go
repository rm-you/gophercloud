...
func TestBrowserOpenerFailureIsReturned(t *testing.T) {
	...
	result := authenticateWebSSO(context.Background(), client, &V3WebSSOOpts{
...
func TestWebSSOListenerStartsBeforeBrowser(t *testing.T) {
	...
	result := authenticateWebSSO(context.Background(), serviceClient(fakeKeystone.Endpoint()), &V3WebSSOOpts{
		IdentityProviderName: "my-idp",
		Protocol:             "openid",
		RedirectHost:         "127.0.0.1",
...
func TestInvalidScopeDoesNotOpenBrowser(t *testing.T) {
	...
	var scopeError gophercloud.ErrScopeProjectDomainIDOrProjectDomainName
	if !errors.As(result.Err, &scopeError) {
...
func TestWebSSOCallbackRejectsInvalidRequest(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			port := findAvailablePort(t)
			results := startWebSSO(t, context.Background(), serviceClient(fakeServer.Endpoint()), &V3WebSSOOpts{
				IdentityProviderName: "my-idp",
				Protocol:             "openid",
				RedirectHost:         "127.0.0.1",
				RedirectPort:         port,
