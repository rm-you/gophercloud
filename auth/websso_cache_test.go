...
func TestWebSSOCacheKeySeparatesFlows(t *testing.T) {
	...
	webSSOKey := WebSSOCacheKey(endpoint, &V3WebSSOOpts{
...
func TestWebSSOCacheKeySeparatesNamespaces(t *testing.T) {
	...
	if first, second := WebSSOCacheKey(endpoint, &base), WebSSOCacheKey(endpoint, &other); first == second {
...
func TestWebSSOCacheKeyIgnoresScope(t *testing.T) {
	...
	projectA := base
	projectA.Scope = &Scope{ProjectID: "project-a"}
	projectB := base
	projectB.Scope = &Scope{ProjectID: "project-b"}

	if first, second := WebSSOCacheKey(endpoint, &projectA), WebSSOCacheKey(endpoint, &projectB); first != second {
...
func TestWebSSOCacheReusesUnscopedTokenAcrossProjects(t *testing.T) {
	...
	result = authenticateWebSSO(context.Background(), &client, &projectB)
	th.AssertNoErr(t, result.Err)
	token, err = result.ExtractTokenID()
	th.AssertNoErr(t, err)
...
func TestWebSSOKeepsKnownTokenIDAfterValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.scoped {
				opts.Scope = &Scope{ProjectID: "project-id"}
			}
			if tt.cached {
				opts.TokenCache = cache
				opts.CacheNamespace = "profile"
				key := WebSSOCacheKey(client.Endpoint, opts)
			...
			if tt.cached {
				result = authenticateWebSSO(context.Background(), &client, opts)
			} else {
...
func TestWebSSOCacheValidationCancellationDoesNotOpenBrowser(t *testing.T) {
	...
	cacheKey := WebSSOCacheKey(endpoint, opts)
	...
	cancel()
	result := authenticateWebSSO(ctx, &gophercloud.ServiceClient{
		ProviderClient: newTestProviderClient(),
		Endpoint:       endpoint,
	}, opts)
...
func TestWebSSOCacheHitSkipsBrowser(t *testing.T) {
	...
	cacheKey := WebSSOCacheKey(v3Endpoint, opts)
	...
	result2 := authenticateWebSSO(context.Background(), &client, &opts2)
